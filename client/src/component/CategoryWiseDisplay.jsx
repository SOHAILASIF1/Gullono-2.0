...
function CategoryWiseDisplay({ heading, category, sortOrder }) {
  ...
  const processedData = useMemo(() => {
    let temp = [...data];
    temp = temp.map((item) => ({ ...item, selling: parseInt(item.selling) }));
    if (sortOrder === 'low-to-high') temp.sort((a, b) => a.selling - b.selling);
    else if (sortOrder === 'high-to-low') temp.sort((a, b) => b.selling - a.selling);
    return temp;
  }, [data, sortOrder]);
          ...
          : processedData.map((product, index) => (
              ...
              >
                ...
                <div className="p-4 flex flex-col gap-3">
                  ...
                  {/* ✅ View Details - size/color select karne ke liye detail page par jao */}
                  <button
